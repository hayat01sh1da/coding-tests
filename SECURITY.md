...
| Ruby exercises       | Ruby 4.0.3 (`.ruby-version`)       | Bundler-managed gems per kata; default relies on stdlib.                        |
| Python exercises     | CPython 3.14.5 (`.python-version`) | Requirements are declared per exercise; base templates use stdlib only.         |
| JavaScript exercises | Node v26.0.0 (`.node-version`)     | Use the Node toolchain plus any dependencies listed inside each `package.json`. |
